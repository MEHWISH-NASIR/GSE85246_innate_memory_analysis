...
# GDC tumor_grade is missing for all TCGA-LUSC cases.
# ============================================================
...
grade_audit <- data.frame(
  ...
  Reason =
    "GDC tumor_grade field is unpopulated for TCGA-LUSC",
...
write.csv(
  ...
  file.path(
    outdir,
    "17d_LUSC_grade_availability.csv"
  ),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(
    outdir,
    "17d_LUSC_patient_expression_clinical.csv"
  ),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(
    outdir,
    "17d_LUSC_stage_association.csv"
  ),
  row.names = FALSE
)
...
write.csv(
  ...
  file.path(
    outdir,
    "17d_LUSC_stage_expression_prevalence.csv"
  ),
  row.names = FALSE
)
...
cat("\n========================================\n")
cat("LUSC STAGE-SPECIFIC EXPRESSION / PREVALENCE\n")
cat("========================================\n\n")
