# ============================================================
# 13j — TCGA-KIRC survival metadata audit
# ============================================================
...
cat("Downloading TCGA-KIRC survival metadata...\n")
...
rows <- lapply(hits, function(case) {
  ...
  # diagnosis-level last follow-up
  diag_fu <- NA_real_
  ...
  # deceased -> days to death
  # alive    -> greatest available follow-up
  os_days <- ifelse(
    event == 1,
    days_death,
    ifelse(
...
# Invalid zero/negative follow-up is not useful for survival
surv$OS_days[
  !is.na(surv$OS_days) &
  surv$OS_days <= 0
] <- NA
...
outfile <- file.path(
  outdir,
  "13j_KIRC_survival_metadata.csv"
)
...
cat("\nPatients with diagnosis follow-up:",
    sum(!is.na(surv$diagnosis_last_followup_days)),
    "\n")

cat("Patients with newer follow-up records:",
    sum(!is.na(surv$case_followup_days)),
    "\n")

cat("Patients with usable OS data:",
